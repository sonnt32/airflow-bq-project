-- L2 obby1_layer2.evt_os_update  <-  os_update  (20260901)
CREATE TABLE IF NOT EXISTS `obby1_layer2.evt_os_update`
...
SELECT
    ...
    (SELECT COALESCE(p.value.string_value, CAST(p.value.int_value AS STRING), CAST(p.value.double_value AS STRING), CAST(p.value.float_value AS STRING)) FROM UNNEST(e.event_params) p WHERE p.key = 'previous_os_version' LIMIT 1) AS previous_os_version
FROM `suvival-master-obby-parkour.analytics_505759684.events_20260901` AS e
WHERE e.event_name = 'os_update'
LIMIT 0;

DELETE FROM `obby1_layer2.evt_os_update`
WHERE event_date = DATE('2026-09-01');

INSERT INTO `obby1_layer2.evt_os_update`
SELECT
    e.user_pseudo_id,
    e.user_id,
    e.event_name,
