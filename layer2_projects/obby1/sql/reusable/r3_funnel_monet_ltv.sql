-- ============================================================
-- New User Funnel Monetization + LTV D0/D3/D7 — chạy trên LAYER 2
-- Nguồn: obby1_layer2.evt_*  (đã transform, mỗi event 1 bảng)
--
-- KHÁC bản raw:
--   - Không EXECUTE IMMEDIATE / không _TABLE_SUFFIX / không var_dataset_path:
--     tên bảng cố định, "range ngày" = số partition đang có trong L2.
--     >>> L2 hiện chỉ có ngày 20260901 => num_days = 1, LTV chỉ ra được D0.
--         Muốn D3/D7 phải build L2 cho 20260901..20260908 trước.
--   - evt_ad_impression KHÔNG có cột app_version (thiết kế không lặp cột)
--     => version lấy theo cohort (INNER JOIN user_identity).
--   - `value` đã là FLOAT64 sẵn -> khỏi bóc event_params.
--   - `ad_format` giữ nguyên gốc -> vẫn cần UPPER() trước STARTS_WITH.
--   - "user active" đọc thẳng evt_session_start (khỏi quét mọi event).
-- ============================================================
