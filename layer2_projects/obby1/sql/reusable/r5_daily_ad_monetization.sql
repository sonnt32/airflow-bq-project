-- ============================================================
-- [R5] Daily Ad Monetization Dashboard — Imp/DAU, Viewer Rate, eCPM, ARPDAU, Cohort RR D1
-- Nguồn raw: memory skills.md "Thống kê về Các chỉ số Moent hàng ngày"
-- KHÁC raw:
--   - bỏ EXECUTE IMMEDIATE / _TABLE_SUFFIX; version lấy từ evt_first_open.
--   - ad_format chuẩn hoá bằng STARTS_WITH (BANNER/MREC/INTER/REWARDED) thay cho LIKE '%...%'.
--   - "is_active_today" = có evt_session_start (chuẩn Data Foundation), KHÔNG dùng user_engagement.
--   - cohort_rr_d1 cần >= 2 ngày trong L2 => hiện = NULL.
-- ============================================================
DECLARE var_versions    ARRAY<STRING> DEFAULT [];              -- rỗng = mọi version
DECLARE var_cohort_event STRING       DEFAULT 'first_open';    -- 'first_open' | 'session_start'
...
user_identity AS (
  ...
  FROM (
    ...
    FROM `obby1_layer2.evt_first_open`    WHERE var_cohort_event = 'first_open'
    UNION ALL
    SELECT user_pseudo_id, event_date AS join_date, app_version
    FROM `obby1_layer2.evt_session_start` WHERE var_cohort_event = 'session_start'
  )
...
joined_data AS (
  ...
  WHERE d.is_active_today = 1
    AND (ARRAY_LENGTH(var_versions) = 0 OR uv.app_version IN UNNEST(var_versions))
)
SELECT
  log_date AS date,
