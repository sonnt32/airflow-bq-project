-- L2 obby1_layer2.evt_mode_clicked  <-  mode_clicked  (20260901)
CREATE TABLE IF NOT EXISTS `obby1_layer2.evt_mode_clicked`
...
SELECT
    ...
    (SELECT COALESCE(p.value.string_value, CAST(p.value.int_value AS STRING), CAST(p.value.double_value AS STRING), CAST(p.value.float_value AS STRING)) FROM UNNEST(e.event_params) p WHERE p.key = 'mode_id' LIMIT 1) AS mode_id
FROM `suvival-master-obby-parkour.analytics_505759684.events_20260901` AS e
WHERE e.event_name = 'mode_clicked'
LIMIT 0;

DELETE FROM `obby1_layer2.evt_mode_clicked`
WHERE event_date = DATE('2026-09-01');

INSERT INTO `obby1_layer2.evt_mode_clicked`
SELECT
    e.user_pseudo_id,
    e.user_id,
    e.event_name,
